...
public class PathingMain extends PApplet
{
    ...
    */
    ...
    private boolean dfs(Point pos, GridValues[][] grid, List<Point> path){
        ...
        if (isValidPath(right, grid, path)) {
            path.add(0, pos);
            return true;
        } else if (isValidPath(up, grid, path)) {
            path.add(0, pos);
            return true;
        } else if (isValidPath(down, grid, path)){
            path.add(0, pos);
            return true;
        } else if (isValidPath(left, grid, path)) {
            path.add(0, pos);
            return true;
        }
        return false;
    ...
    private boolean isValidPath(Point pt, GridValues[][] grid, List<Point> path){
        if (validPt(pt, grid)){
            if (grid[pt.y][pt.x] == GridValues.GOAL){
                path.add(0, pt);
                return true;
            }
            return dfs(pt, grid, path);
        }
        return false;
    }


    private boolean validPt(Point p, GridValues[][] grid){
        return withinBounds(p, grid)  &&
                grid[p.y][p.x] != GridValues.OBSTACLE &&
